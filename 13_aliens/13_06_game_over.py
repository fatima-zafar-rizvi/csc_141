...
try:
    ...
    (ALIEN_WIDTH, ALIEN_HEIGHT))  # Scale to the desired size
except pygame.error as e:
...
def check_ship_collision():
    global ship_hits
    for alien in aliens:
        if alien.colliderect(pygame.Rect(ship_x, ship_y, SHIP_WIDTH, 
        SHIP_HEIGHT)):
...
while True:
    ...
    if all(alien.x < 0 for alien in aliens):  
        print(f"Game Over! "
        f"You hit {alien_hits} aliens and the ship was hit {ship_hits} times.")
        break
    ...
    score_text = font.render(
        f"Aliens Hit: {alien_hits}   Ship Hits: {ship_hits}", True, WHITE)
