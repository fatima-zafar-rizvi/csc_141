...
TARGET_WIDTH = 40
TARGET_HEIGHT = 40
TARGET_SPEED = 2  # Initial speed
original_target_speed = TARGET_SPEED  # Save the original speed
...
def check_play_button():
    global game_running, misses, hits, TARGET_SPEED
    ...
    if button_x <= mouse_x <= button_x + button_width and button_y <= mouse_y <= button_y + button_height:
        if mouse_click[0]:  # Left click
            misses = 0
            hits = 0
            TARGET_SPEED = original_target_speed  # Reset target speed
            game_running = True
...
def start_game():
    global game_running, target_y, target_direction, misses, hits, TARGET_SPEED
    game_running = True
    target_y = SCREEN_HEIGHT // 2
    target_direction = 1
    misses = 0
...
while True:
    ...
    # Gradually increase target speed as the game progresses
    TARGET_SPEED += 0.001  # Increase speed over time
